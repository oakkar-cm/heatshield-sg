      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-800">
          ...
          <div className="text-sm">
            ...
            <p className="mt-1">
              Allow this site in Google Cloud → API key → HTTP referrers:
              {" "}https://heatshieldsg.vercel.app/* and enable Maps JavaScript API.
              Live station data is shown below meanwhile.
            </p>
          </div>
        </div>
        ...
        <h3 className="font-heading font-semibold text-lg mt-6 mb-3 flex items-center gap-2">
          <Snowflake className="w-5 h-5 text-primary" /> Cool spots
        </h3>
        <div className="grid sm:grid-cols-2 gap-2">
          {coolingSpots.map((c) => (
...
export default function MapView({ userLocation, wbgtStations = [], coolingSpots = [], rainStations = [], showRain = false }) {
  ...
  if (!process.env.REACT_APP_GOOGLE_MAPS_API_KEY || authFailed) {
    return <MapFallback wbgtStations={wbgtStations} coolingSpots={coolingSpots} />;
  }
